...
static std::string filename = "../analytics/power_iso.hdf5";
// static std::string filename = "./power_iso.hdf5";
    ...
    for (int k=ks; k<=ke; ++k) {
      for (int j=js; j<=je; ++j) { 
        for (int i=1; i<=(NGHOST); ++i) {
          ...
          // u_cr(CRF1,k,j,is-i) = gc*u_cr(CRE,k,j,is-i)*(w(IVX,k,j,is-i) + pcr->B/sqrt(w(IDN,k,j,is-i)))/pcr->vmax;
    ...
    for (int k=ks; k<=ke; ++k) {
      for (int j=js; j<=je; ++j) {
        for (int i=1; i<=(NGHOST); ++i) {
          ...
          //   u_cr(CRF1,k,j,ie+i) = u_cr(CRF1,k,j,ie+i-1) + (x1 - x2)*grad_fc; 
          // }else{
