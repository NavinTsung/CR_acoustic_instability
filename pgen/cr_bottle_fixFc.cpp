...
static Real rho_c1 = 0.1;
static Real rho_c2 = 1.0;
static Real rho_c3 = 0.5;
static Real rho_extra = 5.0;
static Real rho_h = 0.1;
static Real x01 = 200.0;
static Real x02 = 400.0;
static Real xextra = 0.0;
static Real dx = 25.0;
static Real dxextra = 50.0;
static Real B = 1.0; 
static Real va = B/sqrt(rho_c2);
static Real sigma = 1.e5;
static Real Ec_left = 3.0;
static Real Fc_left = gc*Ec_left*va;

static Real t0 = 2000.0;
static Real t1 = 4000.0;
  ...
  for(int k=ks; k<=ke; ++k) {
    for (int j=js; j<=je; ++j) {
      for (int i=is; i<=ie; ++i) {
        ...
        // rho += (rho_extra - rho_h)*(1. + tanh((x1 - xextra)/dxextra))*(1. - tanh((x1 - xextra)/dxextra));
        Real pg = 1.0;
     ...
     int ks, int ke, int ngh)
{
  ...
  if (time >= t0) {
    if (time <= t1) {
      rho_c = ((rho_c3 - rho_c2)*time + (rho_c2*t1 - rho_c3*t0))/(t1 - t0);
    } else {
      rho_c = rho_c3;
    }
    for (int k=ks; k<=ke; ++k) {
      for (int j=js; j<=je; ++j) {
        for (int i=is; i<=ie; ++i) {
          Real x1 = pco->x1v(i);
          Real rho = rho_h + (rho_c1 - rho_h)*(1. + tanh((x1 - x01)/dx))*(1. - tanh((x1 - x01)/dx));
          rho += (rho_c - rho_h)*(1. + tanh((x1 - x02)/dx))*(1. - tanh((x1 - x02)/dx));
          // rho += (rho_extra - rho_h)*(1. + tanh((x1 - xextra)/dxextra))*(1. - tanh((x1 - xextra)/dxextra));
          prim(IDN,k,j,i) = rho;
        } 
      }
