...
static std::string filename = "../analytics/power_sph.hdf5";
// static std::string filename = "./power_sph.hdf5";
    ...
    for (int k=ks; k<=ke; ++k) {
      for (int j=js; j<=je; ++j) {
        for (int i=is; i<=ie+1; ++i) {
          pfield->b.x1f(k,j,i) = pcr->B/SQR(pcoord->x1f(i));
        }
      }
    }
  ...
  for (int k=ks; k<=ke; ++k) {
    for (int j=js; j<=je; ++j) {
      for (int i=1; i<=(NGHOST); ++i) {
        ...
        prim(IVX,k,j,is-i) = prim(IVX,k,j,is)*prim(IDN,k,j,is)*SQR(xs/x1)/prim(IDN,k,j,is-i);
        prim(IVY,k,j,is-i) = 0.;
        prim(IVZ,k,j,is-i) = 0.;
  ...
  for (int k=ks; k<=ke; ++k) {
    for (int j=js; j<=je; ++j) {
      for (int i=1; i<=(NGHOST); ++i) {
        ...
        prim(IVX,k,j,ie+i) = prim(IVX,k,j,ie)*prim(IDN,k,j,ie)*SQR(xe/x1)/prim(IDN,k,j,ie+i);
