...
void MeshBlock::ProblemGenerator(ParameterInput *pin)
{
  ...
  // Cooling buffer
  pcr->cool_buffer = pin->GetOrAddReal("problem","cool_buffer",0.1);
    ...
    int js, int je, int ks, int ke, int ngh)
{
  if (CR_ENABLED) {
    for (int k=ks; k<=ke; ++k) {
      for (int j=js; j<=je; ++j) { 
        for (int i=1; i<=(NGHOST); ++i) {
          Real ec = pcr->ec_in(is-i);
          Real fc = pcr->fc_in(is-i);
          
          u_cr(CRE,k,j,is-i) = pcr->ec_in(is-i);
          u_cr(CRF1,k,j,is-i) = pcr->fc_in(is-i)/pcr->vmax;
          // u_cr(CRF1,k,j,is-i) = gc*u_cr(CRE,k,j,is-i)*(w(IVX,k,j,is-i) + pcr->B/sqrt(w(IDN,k,j,is-i)));
  ...
  AthenaArray<Real> &cons)
{
  ...
  Real xmin_cool_buffer = xmin + pcr->cool_buffer*domain_size;
  Real xmax_cool_buffer = xmax - pcr->cool_buffer*domain_size;
  ...
  for (int k=kl; k<=ku; ++k) {
    for (int j=jl; j<=ju; ++j) {
      for (int i=il; i<=iu; ++i) {
        ...
        Real temp_new;
        if (((x1 > xmin) && (x1 < xmin_cool_buffer)) || ((x1 < xmax) && (x1 > xmax_cool_buffer))) {
