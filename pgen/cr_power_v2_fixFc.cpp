...
void MeshBlock::ProblemGenerator(ParameterInput *pin)
{
  ...
  // couple time
  pcr->couple_timing = pin->GetOrAddReal("problem","couple_time",0.0);
    ...
    for (int k=ks; k<=ke; ++k) {
      for (int j=js; j<=je; ++j) {
        for (int i=1; i<=(NGHOST); ++i) {
          ...
          // if (grad_ec < -TINY_NUMBER) {
          //   u_cr(CRE,k,j,ie+i) = u_cr(CRE,k,j,ie+i-1) + grad_ec*(x1 - x2);
          // } else {
          //   u_cr(CRE,k,j,ie+i) = 0.99*u_cr(CRE,k,j,ie+i-1);
          // }
          u_cr(CRE,k,j,ie+i) = u_cr(CRE,k,j,ie+i-1) + grad_ec*(x1 - x2);
  ...
  for (int k=kl; k<=ku; ++k) {
    for (int j=jl; j<=ju; ++j) {
      for (int i=il; i<=iu; ++i) {
        ...
        Real new_temp = temp + (gg - 1.)*pcr->H_in(i)*dt/rho;

        // Enforce temperature floor
        new_temp = std::max(new_temp, pcr->Tfloor);
        new_temp = std::min(new_temp, pcr->Tceil);

        Real mom_src = prim(IDN,k,j,i)*pcr->g_in(i);
        Real eng_src = mom_src*prim(IVX,k,j,i);

        // Update momentum and energy
        if (time > pcr->couple_timing) {
          cons(IM1,k,j,i) += mom_src*dt;
          cons(IEN,k,j,i) += eng_src*dt + (new_temp - temp)*rho/(gg - 1.);
        }
