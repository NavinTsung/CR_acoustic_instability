  ...
  for (int k=ks; k<=ke; ++k) {
    for (int j=js; j<=je; ++j) {
      for (int i=is; i<=ie; ++i) {
        ...
        Real vel = 1.0/pcr->rho_in(i);
        
        // Initialize hydro variables
        phydro->u(IDN,k,j,i) = pcr->rho_in(i);
        phydro->u(IM1,k,j,i) = pcr->rho_in(i)*vel;
        phydro->u(IM2,k,j,i) = 0.0;
        phydro->u(IM3,k,j,i) = 0.0;
        if (NON_BAROTROPIC_EOS) {
          phydro->u(IEN,k,j,i) = 0.5*pcr->rho_in(i)*SQR(vel) + pcr->pg_in(i)/(gg - 1.);
        }
    ...
    for (int k=ks; k<=ke; ++k) {
      for (int j=js; j<=je; ++j) { 
        for (int i=1; i<=(NGHOST); ++i) {
          ...
          Real grad_fc = (u_cr(CRF1,k,j,is-i+2) - u_cr(CRF1,k,j,is-i+1))/(x3 - x2);
          ...
          // u_cr(CRF1,k,j,is-i) = u_cr(CRF1,k,j,is-i+1) + grad_fc*(x1 - x2);
