    ...
    for (int k=ks; k<=ke; ++k) {
      for (int j=js; j<=je; ++j) {
        for (int i=1; i<=(NGHOST); ++i) {
          ...
          // u_cr(CRF1,k,j,ie+i) = fc/pcr->vmax;
  ...
  for (int k=kl; k<=ku; ++k) {
    for (int j=jl; j<=ju; ++j) {
      for (int i=il; i<=iu; ++i) {
        ...
        // Calculate new temperature due to cooling using the Townsend Algorithm
        Real temp_new = cooler.townsend(temp, rho, dt);

        // New temperature including heating
        temp_new += cooler.heating(pcr->H_in(i), dt);

        // If heating is more than cooling at the lower end, numerical issue, set to tfloor
        // if ((temp_new > temp) && (temp_new*unit_temp < 1.1e4)) {temp_new = cooler.tfloor/unit_temp;}
