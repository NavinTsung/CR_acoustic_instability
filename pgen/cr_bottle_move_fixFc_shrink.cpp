...
static Real x01 = 20.0;
static Real x02 = 80.0;
static Real x03 = 80.0;
static Real dx = 2.5;
static Real B = 1.0; 
static Real pg = 1.0;
static Real va = B/sqrt(rho_h);
static Real v0 = 0.3*va;
...
static Real t0 = 200.0;
static Real t1 = 500.0;
  ...
  if (time >= t0) {
    for (int k=ks; k<=ke; ++k) {
      for (int j=js; j<=je; ++j) {
        for (int i=is; i<=ie; ++i) {
          Real x11 = pco->x1v(i);
          Real x12 = pco->x1v(i) + 0.2*v*(time - t0);
          Real x13 = pco->x1v(i);
