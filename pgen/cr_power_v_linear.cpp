...
void MeshBlock::ProblemGenerator(ParameterInput *pin)
{
  ...
  // Passive Scalars
  Real passive_spread, length, x_passive_begin, x_passive_end;
  if (NSCALARS > 0) {
    passive_spread = pin->GetOrAddReal("problem","passive_spread", 0.05);
    length = pcr->x_right - pcr->x_left;
    x_passive_begin = pcr->x_left + (0.5 - passive_spread/2.)*length;
    x_passive_end = pcr->x_left + (0.5 + passive_spread/2.)*length;
  } 
  ...
  for (int k=ks; k<=ke; ++k) {
    for (int j=js; j<=je; ++j) {
      for (int i=is; i<=ie; ++i) {
        ...
        // Real dv = 1.e-3*exp(-SQR((x1 - 1.5)/ldiff));
    ...
    for (int j=js; j<=je; ++j) {
      for (int i=1; i<=(NGHOST); ++i) {
        for (int n=0; n<(NHYDRO); ++n) {
          ...
          Real vx0 = pcr->v_in(is);
          Real cs0 = sqrt(gg*pcr->pg_in(is)/pcr->rho_in(is));
          Real dvx = amp_left*cs0*growth*sin(k_wv*(x1 - x_left) - k_wv*(vx0 + cs0)*time);
          Real vx = pcr->v_in(is-i) + dvx;
          Real rho = pcr->rho_in(is-i)*(1. + dvx/cs0);
          Real pg = pcr->pg_in(is-i)*(1. + gg*dvx/cs0);
          if (n==(IDN)) {
    ...
    for (int j=js; j<=je; ++j) {
      for (int i=1; i<=(NGHOST); ++i) {
        for (int n=0; n<(NHYDRO); ++n) {
          ...
          Real vx0 = pcr->v_in(ie);
          Real cs0 = sqrt(gg*pcr->pg_in(ie)/pcr->rho_in(ie));
          Real dvx = amp_right*cs0*growth*sin(k_wv*(x1 - x_right) - k_wv*(vx0 - cs0)*time);
          Real vx = pcr->v_in(ie+i) + dvx;
          Real rho = pcr->rho_in(ie+i)*(1. - dvx/cs0);
          Real pg = pcr->pg_in(ie+i)*(1. - gg*dvx/cs0);
          if (n==IDN) {
