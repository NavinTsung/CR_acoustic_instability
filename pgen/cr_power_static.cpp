  ...
  for (int k=ks; k<=ke; ++k) {
    for (int j=js; j<=je; ++j) {
      for (int i=1; i<=(NGHOST); ++i) {
        ...
        // Real grad_rho = (prim(IDN,k,j,is-i+2) - prim(IDN,k,j,is-i+1))/(x3 - x2);
        // Real grad_pg = (prim(IPR,k,j,is-i+2) - prim(IPR,k,j,is-i+1))/(x3 - x2);

        // if (prim(IVX,k,j,is) >= 0) {
        //   prim(IDN,k,j,is-i) = pcr->rho_in(is-i);
        //   prim(IVX,k,j,is-i) = 0.;
        //   prim(IPR,k,j,is-i) = pcr->pg_in(is-i);
        // } else {
        //   prim(IDN,k,j,is-i) = prim(IDN,k,j,is-i+1) + grad_rho*(x1 - x2);
        //   prim(IVX,k,j,is-i) = prim(IVX,k,j,is)*prim(IDN,k,j,is)/prim(IDN,k,j,is-i);
        //   prim(IPR,k,j,is-i) = prim(IPR,k,j,is-i+1) + grad_pg*(x1 - x2);
        // }
        prim(IDN,k,j,is-i) = pcr->rho_in(is-i);
        // prim(IVX,k,j,is-i) = 0.;
  ...
  for (int k=ks; k<=ke; ++k) {
    for (int j=js; j<=je; ++j) {
      for (int i=1; i<=(NGHOST); ++i) {
        ...
        // if (prim(IVX,k,j,ie) > 0) {
        //   prim(IDN,k,j,ie+i) = prim(IDN,k,j,ie+i-1) + grad_rho*(x1 - x2);
        //   prim(IVX,k,j,ie+i) = prim(IVX,k,j,ie)*prim(IDN,k,j,ie)/prim(IDN,k,j,ie+i);
        //   prim(IPR,k,j,ie+i) = prim(IPR,k,j,ie+i-1) + grad_pg*(x1 - x2);
        // } else {
  ...
  for (int k=kl; k<=ku; ++k) {
    for (int j=jl; j<=ju; ++j) {
      for (int i=il; i<=iu; ++i) {
        ...
        Real new_temp = temp + (gg - 1.)*pcr->H_in(i)*dt;
