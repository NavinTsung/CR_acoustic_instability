...
static std::string filename = "../analytics/power_cool.hdf5";
// static std::string filename = "./power_cool.hdf5";
...
void InnerBoundaryPS(MeshBlock *pmb, Coordinates *pco, PassiveScalars *pscalars, 
    const AthenaArray<Real> &w, const AthenaArray<Real> &bcc,
    AthenaArray<Real> &r, Real time, Real dt, int is, int ie, 
    int js, int je, int ks, int ke, int ngh);

void OuterBoundaryPS(MeshBlock *pmb, Coordinates *pco, PassiveScalars *pscalars, 
    const AthenaArray<Real> &w, const AthenaArray<Real> &bcc,
    AthenaArray<Real> &r, Real time, Real dt, int is, int ie, 
    int js, int je, int ks, int ke, int ngh);
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  if (mesh_bcs[BoundaryFace::inner_x1] == GetBoundaryFlag("user")) {
    ...
    if(NSCALARS > 0){
      EnrollUserPSBoundaryFunction(BoundaryFace::inner_x1, InnerBoundaryPS);
    }
  }
  if (mesh_bcs[BoundaryFace::outer_x1] ==  GetBoundaryFlag("user")) {
    ...
    if(NSCALARS > 0){
      EnrollUserPSBoundaryFunction(BoundaryFace::outer_x1, OuterBoundaryPS);
    }
  }
...
void MeshBlock::ProblemGenerator(ParameterInput *pin)
{
  ...
  hid_t attr_L0 = H5Aopen(file, "L0", H5P_DEFAULT);
  ...
  hid_t dset_Hnocool = H5Dopen(file, "Hnocool", H5P_DEFAULT);
  ...
  hid_t fspace_Hnocool = H5Dget_space(dset_Hnocool);
  ...
  H5Sselect_hyperslab(fspace_Hnocool, H5S_SELECT_SET, start_file, NULL, count_file, NULL);
  ...
  H5Aread(attr_B, H5T_NATIVE_DOUBLE, &(pcr->B));
  H5Aread(attr_kappa, H5T_NATIVE_DOUBLE, &(pcr->kap));
  H5Aread(attr_ldiff, H5T_NATIVE_DOUBLE, &ldiff);
  H5Aread(attr_T0, H5T_NATIVE_DOUBLE, &(pcr->T0));
  H5Aread(attr_L0, H5T_NATIVE_DOUBLE, &(pcr->L0));
  ...
  H5Dread(dset_Hnocool, H5T_NATIVE_DOUBLE, mspace, fspace_Hnocool, xfer_plist, &(pcr->Hnocool_in(0)));
  ...
  // Passive Scalars
  Real passive_start, passive_spread, length, x_passive_begin, x_passive_end;
  if (NSCALARS > 0) {
    passive_start = pin->GetOrAddReal("problem","passive_start",0.01);
    passive_spread = pin->GetOrAddReal("problem","passive_spread",0.01);
    length = pcr->x_right - pcr->x_left;
    x_passive_begin = pcr->x_left + (passive_start - passive_spread/2.)*length;
    x_passive_end = pcr->x_left + (passive_start + passive_spread/2.)*length;
  } 

  pcr->Tfloor = pin->GetOrAddReal("problem","T_floor",0.01);
  pcr->Tceil = pin->GetOrAddReal("problem","T_ceil",100.0);
  pcr->epsil = pin->GetOrAddReal("problem","epsilon",-0.66667);

  // Viscosity
  pcr->viscos_buffer_left = pin->GetOrAddReal("problem","viscos_buffer_left",0.1);
  pcr->viscos_buffer_right = pin->GetOrAddReal("problem","viscos_buffer_right",0.1);

  // Cooling buffer
  pcr->cool_buffer_left = pin->GetOrAddReal("problem","cool_buffer_left",0.1);
  pcr->cool_buffer_right = pin->GetOrAddReal("problem","cool_buffer_right",0.1);
      ...
      for (int i=is; i<=ie; ++i) {
        ...
        if (NSCALARS > 0) {
          for (int n=0; n<NSCALARS; ++n) {
            if ((x1 >= x_passive_begin) && (x1 <= x_passive_end)) {
              pscalars->s(n,k,j,i) = pcr->rho_in(i);
            } else {
        ...
        if (NON_BAROTROPIC_EOS) {
          phydro->u(IEN,k,j,i) = 0.5*pcr->rho_in(i)*SQR(pcr->v_in(i)) + pcr->pg_in(i)/(gg - 1.);
        }
     ...
     int ks, int ke, int ngh)
{
  ...
  for (int k=ks; k<=ke; ++k) {
    for (int j=js; j<=je; ++j) {
      for (int i=1; i<=(NGHOST); ++i) {
        Real rho = pcr->rho_in(is-i);
        // Real v = pcr->v_in(is-i);
        Real pg = pcr->pg_in(is-i);
        
        prim(IDN,k,j,is-i) = rho;
        // prim(IDN,k,j,is-i) = prim(IDN,k,j,is);
        prim(IVX,k,j,is-i) = prim(IVX,k,j,is)*prim(IDN,k,j,is)/prim(IDN,k,j,is-i);
        // prim(IVX,k,j,is-i) = v;
        prim(IVY,k,j,is-i) = 0.;
        prim(IVZ,k,j,is-i) = 0.;
        prim(IPR,k,j,is-i) = pg;
  ...
  for (int k=ks; k<=ke; ++k) {
    for (int j=js; j<=je; ++j) {
      for (int i=1; i<=(NGHOST); ++i) {
        ...
        // Real grad_rho = (prim(IDN,k,j,ie+i-1) - prim(IDN,k,j,ie+i-2))/(x2 - x3);
        // Real grad_v = (prim(IVX,k,j,ie+i-1) - prim(IVX,k,j,ie+i-2))/(x2 - x3);
        // Real grad_pg = (prim(IPR,k,j,ie+i-1) - prim(IPR,k,j,ie+i-2))/(x2 - x3);
        // Real rho = prim(IDN,k,j,ie+i-1) + grad_rho*(x1 - x2);
        // Real v = prim(IVX,k,j,ie+i-1) + grad_v*(x1 - x2);
        // Real v = prim(IVX,k,j,ie+i-1)*prim(IDN,k,j,ie+i-1)/prim(IDN,k,j,ie+i);
        // Real pg = prim(IPR,k,j,ie+i-1) + grad_pg*(x1 - x2);
        
        // prim(IDN,k,j,ie+i) = pcr->rho_in(ie+i);
        prim(IDN,k,j,ie+i) = prim(IDN,k,j,ie);
        // prim(IDN,k,j,ie+i) = rho;
        prim(IVX,k,j,ie+i) = prim(IVX,k,j,ie)*prim(IDN,k,j,ie)/prim(IDN,k,j,ie+i);
        // prim(IVX,k,j,ie+i) = pcr->v_in(ie+i);
        prim(IVY,k,j,ie+i) = 0.0;
        prim(IVZ,k,j,ie+i) = 0.0;
        // prim(IPR,k,j,ie+i) = pcr->pg_in(ie+i);
        // prim(IPR,k,j,ie+i) = pg;
    ...
    for (int k=ks; k<=ke; ++k) {
      for (int j=js; j<=je; ++j) { 
        for (int i=1; i<=(NGHOST); ++i) {
          ...
          Real x3 = pco->x1v(is-i+2);

          Real grad_ec = (u_cr(CRE,k,j,is-i+2) - u_cr(CRE,k,j,is-i+1))/(x3 - x2);
          
          u_cr(CRE,k,j,is-i) = u_cr(CRE,k,j,is-i+1) + grad_ec*(x1 - x2);
    ...
    for (int k=ks; k<=ke; ++k) {
      for (int j=js; j<=je; ++j) {
        for (int i=1; i<=(NGHOST); ++i) {
          ...
          // Real fc = gc*u_cr(CRE,k,j,ie+i)*(w(IVX,k,j,ie+i) + pcr->B/sqrt(w(IDN,k,j,ie+i)));

          // u_cr(CRE,k,j,ie+i) = u_cr(CRE,k,j,ie);
          ...
          // u_cr(CRF1,k,j,ie+i) = u_cr(CRF1,k,j,ie+i-1) + (x1 - x2)*(w(IVX,k,j,ie+i) + pcr->B/sqrt(w(IDN,k,j,ie+i)))*grad_ec/(gc - 1.)/pcr->vmax;
...
{
  CosmicRay *pcr = pmb->pcr;
  Real xmin = pcr->x_left;
  Real xmax = pcr->x_right;
  Real domain_size = xmax - xmin;
  Real xmin_cool_buffer_left = xmin + pcr->cool_buffer_left*domain_size;
  Real xmax_cool_buffer_right = xmax - pcr->cool_buffer_right*domain_size;

  // Cooling
  Cooling cooler(pcr->Tfloor, pcr->Tceil, pcr->T0, pcr->L0, pcr->epsil);
  ...
  for (int k=kl; k<=ku; ++k) {
    for (int j=jl; j<=ju; ++j) {
      for (int i=il; i<=iu; ++i) {
        ...
        Real temp_new;
        if (((x1 > xmin) && (x1 < xmin_cool_buffer_left)) || ((x1 < xmax) && (x1 > xmax_cool_buffer_right))) {
          temp_new = temp + (gg - 1.)*pcr->Hnocool_in(i)*dt/rho;
        } else {
          temp_new = cooler.townsend(temp, rho, dt);
          // temp_new += cooler.heating(pcr->H_in(i), dt)/rho;
          if (pcr->H_in(i) > 0) {
            temp_new += cooler.heating(pcr->H_in(i), dt)/rho;
          }
        }
...
void Viscosity(HydroDiffusion *phdif, MeshBlock *pmb, const AthenaArray<Real> &w,
  const AthenaArray<Real> &bc, int is, int ie, int js, int je, int ks, int ke)
  {
    ...
    Real xmax = pcr->x_right;
    Real domain_size = xmax - xmin;
    Real xmin_vis_buffer = xmin + pcr->viscos_buffer_left*domain_size;
    Real xmax_vis_buffer = xmax - pcr->viscos_buffer_right*domain_size;
