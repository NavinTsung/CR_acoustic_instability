...
class Acous1d:
  ...
  def growthrate(self):
    ...
    if self.forward:
      ...
      acous = -0.5*(cc**2/kappa)*(acous1 + acous2)
    else:
      acous1 = (1. - (gg - 1.)*vs/cs)*(1. + 0.5*vs/cs)
      acous2 = (kappa/(gc*Lc*cs))*(1. - 0.5*(gg - 1.)*vs/cs)
      acous = -0.5*(cc**2/kappa)*(acous1 - acous2)
...
class AcousProfile:
  ...
  def amplitude(self, amp0=1.e-5, log=True):
    ...
    if self.forward:
      ...
      amp_genuine[-1] = amp0
      for i in np.arange(1, np.size(x)):
        amp[-(i+1)] = amp0*np.exp(0.5*np.log(rho[-1]/rho[i]) + np.sum(growth[::-1][:(i+1)]*dx/sound_speed[::-1][:(i+1)]))
...
prof = AcousProfile(profile, forward=False) 
fig = prof.plot()
fig.savefig('/Users/tsunhinnavintsung/Workspace/Codes/workspace/1dcr_v1_2/cr_acous/power/results/analytics/profile.png', dpi=300)
fig1 = prof.amplitude(amp0=1.e-4, log=True)
np.savetxt('/Users/tsunhinnavintsung/Workspace/Codes/workspace/1dcr_v1_2/cr_acous/power/results/analytics/x.csv', prof.x)
np.savetxt('/Users/tsunhinnavintsung/Workspace/Codes/workspace/1dcr_v1_2/cr_acous/power/results/analytics/amp.csv', prof.amp)
plt.show()
plt.close('all')
