...
class Acous1d:
  def __init__(self, equil, forward=True):
    ...
    self.dvdx = equil['dvdx']
    self.dpgdx = equil['dpgdx']
  ...
  def growthrate(self):
    ...
    if self.forward:
      acous0 = (v*dpgdx + gg*pg*dvdx)/pg
      acous1 = (1. + (gg - 1.)*vs/cs)*(1. - 0.5*vs/cs)
      acous2 = (kappa/(gc*Lc*cs))*(1. + 0.5*(gg - 1.)*vs/cs) 
      acous = -0.5*acous0 - 0.5*(cc**2/kappa)*(acous1 + acous2) 
    else:
      acous0 = (v*dpgdx + gg*pg*dvdx)/pg
      acous1 = (1. - (gg - 1.)*vs/cs)*(1. + 0.5*vs/cs)
      acous2 = (kappa/(gc*Lc*cs))*(1. - 0.5*(gg - 1.)*vs/cs)
...
with h5py.File('./power_analysis.hdf5', 'r') as fp:
  ...
  dvdx = np.array(fp['dvdx'])
  dpgdx = np.array(fp['dpgdx'])
...
fig1 = prof.amplitude(amp0=1.8723e-5, log=True)
